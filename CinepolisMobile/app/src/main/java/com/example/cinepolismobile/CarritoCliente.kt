...
class CarritoCliente : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val montoFinal: String =
            (obtenerPrecioComida(listaProductos) + obtenerPrecioBoletos(listaProductos)).toString()
        ...
        finalizarCompra!!.setOnClickListener {
            ...
            if (dataSet!!.next()) {
                ...
                if (outResultCode == 0) {
                    ...
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
                        if (checkSelfPermission(android.Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED) {
                            ...
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                                savePDF(data, idUsuario, montoFinal)
                            }
                        }
                    } else {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    ...
                    Toast.makeText(
                        this,
                        "¡Gracias por su compra ,la factura llegara a su correo electrónico!",
                        Toast.LENGTH_LONG
                    ).show()
                    sendEmail(obtenerEmail(idUsuario))
    ...
    private fun savePDF(data: String, idUsuario: Int, montoTotal: String) {
        val mDoc = com.itextpdf.text.Document()
        val mFileDate = SimpleDateFormat(
            "dd-MM-yyy HH:mm:ss",
            Locale.getDefault()
        ).format(System.currentTimeMillis())
        val mFileName = "Factura_Cinepolis"
        val nombreCliente = obtenerNombreCliente(idUsuario)

        var mFilePath: String = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD_MR1) {
            getActivity(this)?.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS)
        ...
        try {
            ...
            mDoc.add(Paragraph("Cliente: $nombreCliente"))
            mDoc.close()
    ...
    private fun obtenerNombreCliente(idUsuario: Int): String {
        ...
        return if (dataSet!!.next()) {
            dataSet.getString(1) + " " + dataSet.getString(2) + " " + dataSet.getString(3)
        } else {
            ""
        }
    }
    ...
    fun obtenerInfoPdf(listaProductos: List<String>): String {
        ...
        for (producto in listaProductos) {
            ...
            if ("Boleto" in producto) {
                ...
                infoPdf += (producto + " ¢" + obtenerPrecio(
                    infoProducto[1],
                    infoProducto[0].toInt()
                ).toString() + "\n")
            }
        }
        return infoPdf
    ...
    fun sendEmail(email:String){
        var enviarCorreo = JavaMailApi(this,email,"Factura cinépolis")
